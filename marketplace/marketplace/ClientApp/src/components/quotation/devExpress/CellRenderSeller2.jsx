...
function CellRender(props) {
  ...
  if (props.percentageDiscount != null && props.price != null) {
    ...
    initialFee = calculateInitialFee(props.price, props.percentageQuota);
    balance = priceDiscount - initialFee;
  }
    ...
    <React.Fragment>
      <Grid container className={classes.fila}>
        <Grid item xs={2} sm={2} md={2} lg={2}>
          ...
          <label className={classes.labelData}></label>
          <label className={classes.labelData}>
            {t(props.balanceLabel)}
            <span className={classes.spanText}>{balance}</span>
          </label>
        </Grid>
      </Grid>
    </React.Fragment>
